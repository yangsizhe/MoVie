...
def make_env(cfg):
	...
	if domain == 'xarm':
		...
		env = make_env(
    	    ...
    	    action_space='xy' if task=='reach' or task=='push' else 'xyz',  # Reach, Push: 'xy'.  Pegbox, Hammerall: 'xyz'
    	)
	else:
...
def make_env_viewgen(cfg, difficulty=0.0, fov=0, is_shake=False, is_moving=False):
	domain, task = cfg.task.replace('-', '_').split('_', 1)
	if domain == 'xarm':
		env = distracting_control_viewgen.make_env(domain, task, difficulty=difficulty, fov=fov, is_shake=is_shake, is_moving=is_moving, frame_stack=cfg.frame_stack)
	else:
		domain = dict(cup='ball_in_cup').get(domain, domain)
		assert (domain, task) in suite.ALL_TASKS
		env = distracting_control_viewgen.make_env(domain, task, difficulty=difficulty, fov=fov, is_shake=is_shake, is_moving=is_moving)
		env = ActionDTypeWrapper(env, np.float32)
		env = ActionRepeatWrapper(env, cfg.action_repeat)
		env = action_scale.Wrapper(env, minimum=-1.0, maximum=+1.0)
...
def make_env_via_viewgen_type(cfg, viewgen_type):
	domain, task = cfg.task.replace('-', '_').split('_', 1)
	if viewgen_type == 'novel_view':
		env = make_env_viewgen(cfg, difficulty=0.4, fov=50 if domain == 'xarm' else 45, is_shake=False, is_moving=False)
	elif viewgen_type == 'moving_view':
		env = make_env_viewgen(cfg, difficulty=0.6 if domain == 'xarm' else 0.4, fov=50 if domain == 'xarm' else 45, is_shake=False, is_moving=True)
	elif viewgen_type == 'shaking_view':
		env = make_env_viewgen(cfg, difficulty=0.0, fov=50 if domain == 'xarm' else 45, is_shake=True, is_moving=False)
	elif viewgen_type == 'novel_fov':
		env = make_env_viewgen(cfg, difficulty=0.0, fov=60 if domain == 'xarm' else 53, is_shake=False, is_moving=False)
