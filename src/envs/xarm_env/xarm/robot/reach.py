...
from xarm.robot.base import BaseEnv, get_full_asset_path
...
class ReachEnv(BaseEnv, utils.EzPickle):
    ...
    def _get_state_obs(self):
        ...
        values = np.array(
            [self.goal_distance(eef_pos, goal_pos, self.use_xyz), gripper_angle]
        )

        return np.concatenate([eef_pos, eef_velp, goal_pos, values], axis=0)
    ...
    def _sample_goal(self, new=True):
        ...
        if new:
            ...
            goal[0] += self.np_random.uniform(
                -0.05 - 0.05 * self.sample_large,
                0.05 + 0.05 * self.sample_large,
                size=1,
            )
            goal[1] += self.np_random.uniform(
                -0.1 - 0.1 * self.sample_large, 0.1 + 0.1 * self.sample_large, size=1
            )
        else:
