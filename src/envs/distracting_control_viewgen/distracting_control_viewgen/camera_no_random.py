...
def get_camera_params(scale, is_phi=True, is_theta=True):
    return dict(
...
class DistractingCameraEnv(control.Environment):
  ...
  **NOTE**: This wrapper should be applied BEFORE the pixel wrapper to make sure
  ...
  def __init__(self,
    ...
    self._horizontal_delta_origin = horizontal_delta
    self._vertical_delta_origin = vertical_delta
  ...
  def setup_camera(self):      
    ...
    if self._is_moving:
      ...
      self._horizontal_delta = scale * self._horizontal_delta_origin
      self._vertical_delta = scale * self._vertical_delta_origin
    ...
    #                                            self._max_h_angle)
    self._h_angle = self._max_h_angle

    # self._v_angle = self._random_state.uniform(self._min_v_angle,
    #                                            self._max_v_angle)
    self._v_angle = self._min_v_angle
    ...
    #                                             self._max_roll_vel)
    self._roll_vel = self._max_roll_vel
  ...
  def _apply(self):
    ...
    if self._is_shake:
      if self.step_count % 1 == 0:
        self.noise = np.random.normal(loc=0, scale=0.03, size=(3,))
      camera_pos_noisy = cart_cam_pos + self.noise
      cart_cam_pos = np.clip(camera_pos_noisy, cart_cam_pos - 0.07, cart_cam_pos + 0.07)
    ...
    # import pdb;pdb.set_trace()
    if self._fov != 45:
      self._env._physics.model.cam_fovy = self._fov
